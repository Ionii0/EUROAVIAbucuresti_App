    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException, UsernameNotFoundException {
        ...
        try {
            if(StringUtils.hasText(jwt) && jwtProvider.validateToken(jwt)){
                String mailEuroavia=jwtProvider.getMailEuroaviaFromJWT(jwt);

                UserDetails userDetails=userDetailsService.loadUserByUsername(mailEuroavia);
        ...
        } catch (SpringEuroaviaException e) {
            e.printStackTrace();
        }
